...
if (!(((tif)->tif_dir.td_fieldsset[(24) / 32]) & (((unsigned long)1L) << ((24) & 31)))) {
    if ((td->td_planarconfig == 1 && td->td_nstrips > 1) || (td->td_planarconfig == 2 && td->td_nstrips != td->td_samplesperpixel)) {
        MissingRequired(tif, "StripByteCounts");
        goto bad;
    }
    TIFFWarning(module, "%s: TIFF directory is missing required \"%s\" field, calculating from imagelength", tif->tif_name, TIFFFieldWithTag(tif, 279)->field_name);
    if (EstimateStripByteCounts(tif, dir, dircount) < 0)
        goto bad;
} else if (td->td_nstrips == 1 && td->td_stripoffset[0] != 0 && ((td->td_stripbytecount[0] == 0 && td->td_stripoffset[0] != 0) || (td->td_compression == 1 && td->td_stripbytecount[0] > ((*(tif)->tif_sizeproc)((tif)->tif_clientdata)) - td->td_stripoffset[0]) || (tif->tif_mode == 0 && td->td_compression == 1 && td->td_stripbytecount[0] < TIFFScanlineSize(tif) * td->td_imagelength))) {
    TIFFWarning(module, "%s: Bogus \"%s\" field, ignoring and calculating from imagelength", tif->tif_name, TIFFFieldWithTag(tif, 279)->field_name);
    if (EstimateStripByteCounts(tif, dir, dircount) < 0)
        goto bad;
} else {
    if ((td->td_nstrips > 1 && td->td_compression == 1 && td->td_stripbytecount[0] != td->td_stripbytecount[1]) && !(1)) {
