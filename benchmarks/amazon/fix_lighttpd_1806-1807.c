...
if ((!con->conditional_is_valid[dc->comp]) && !((!con->conditional_is_valid[dc->comp]) )) {
    if (con->conf.log_condition_handling) {
        log_trace("%s.%d: (trace) cond[%d] is valid: %d", remove_path("src/configfile-glue.c"), 242, dc->comp, con->conditional_is_valid[dc->comp]);
    }
