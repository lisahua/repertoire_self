...
import sketch.compiler.ast.core.exprs.ExprLiteral;
...
import sketch.compiler.codegenerators.tojava.ExprJavaConstructor;
import sketch.compiler.codegenerators.tojava.StmtIODecl;
import sketch.compiler.passes.lowering.GetExprType;
import sketch.compiler.passes.lowering.SymbolTableVisitor;

public class SketchEqTranfer extends SymbolTableVisitor {

	private Type ctype;
	private String indent = "";
	private int binOpLevel = 0;

	public SketchEqTranfer(SymbolTable symtab) {
		super(symtab);
	}
	...
	public String convertTypeFull(Type type) {
		if (type instanceof TypeArray) {
			TypeArray array = (TypeArray) type;
			return convertTypeFull(array.getBase()) + "[" + (String) array.getLength().accept(this) + "]";
		}
	...
	public String doParams(List params, String prefix) {
		...
		for (Iterator iter = params.iterator(); iter.hasNext();) {
			...
			symtab.registerVar(param.getName(), (param.getType()), param, SymbolTable.KIND_FUNC_PARAM);
			if (!first)
				result += ", ";
			if (prefix != null)
				result += prefix + " ";
	...
	public String doAssignment(Expression lhs, Expression rhs, SymbolTable symtab) {
	...
	public Object visitExprTypeCast(ExprTypeCast exp) {
		return "((" + convertType(exp.getType()) + ")(" + (String) exp.getExpr().accept(this) + "))";
	}
	...
	public Object visitFieldDecl(FieldDecl field) {
		...
		for (int i = 0; i < field.getNumFields(); i++) {
			symtab.registerVar(field.getName(i), (field.getType(i)), field, SymbolTable.KIND_FIELD);
			if (i > 0)
				result += ", ";
	...
	public String outputStructure(StructDef struct) {
		String result = "";
		result += indent + "class " + struct.getName() + " extends Structure {\n";
		addIndent();
		for (Entry<String, Type> entry : struct) {
			result += indent + convertType(entry.getValue()) + " " + entry.getKey() + ";\n";
		}
		unIndent();
		result += indent + "}\n";
		return result;
	...
	public Object visitStmtAssign(StmtAssign stmt) {
		...
		return (String) stmt.getLHS().accept(this) + op + (String) stmt.getRHS().accept(this);
	}
	...
	public Object visitStmtExpr(StmtExpr stmt) {
		...
		if (result.charAt(0) == '(' && Character.isUpperCase(result.charAt(1)))
	...
	public Object visitStmtAssert(StmtAssert stmt) {
		return stmt.getAssertSymbol() + " (" + (String) stmt.getCond().accept(this) + ");";
	}
	...
	public Object visitStmtVarDecl(StmtVarDecl stmt) {
		...
		for (int i = 0; i < stmt.getNumVars(); i++) {
			symtab.registerVar(stmt.getName(i), (stmt.getType(i)), stmt, SymbolTable.KIND_LOCAL);
			if (i > 0)
				result += ", ";
	...
	public Object visitStmtWhile(StmtWhile stmt) {
		...
		return "while (" + (String) stmt.getCond().accept(this) + ") " + (String) stmt.getBody().accept(this);
	}
	...
	public Object visitOther(FENode node) {
		...
		if (node instanceof StmtIODecl) {
			...
			String result = io.getName() + " = new Channel(" + typeToClass(io.getType()) + ", "
