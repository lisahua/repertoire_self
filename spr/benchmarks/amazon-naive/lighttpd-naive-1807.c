...
static cond_result_t config_check_cond_cached(server *srv, connection *con, data_config *dc) {
	...
	if ((COND_RESULT_UNSET == caches[dc->context_ndx].result) || 1) {
	    if (con->conf.log_condition_handling) {
	        log_trace("%s.%d: (trace) === start of %d condition block ===", remove_path("src/configfile-glue.c"), 482, dc->context_ndx);
	    }
	    if (COND_RESULT_TRUE == (caches[dc->context_ndx].result = config_check_cond_nocache(srv, con, dc))) {
	        if (dc->next) {
	            data_config *c;
	            if (con->conf.log_condition_handling) {
	                log_trace("%s.%d: (trace) setting remains of chaining to %s", remove_path("src/configfile-glue.c"), 488, "false");
	            }
	    ...
	    if (con->conf.log_condition_handling) {
	        log_trace("%s.%d: (trace) [%d] result: %s", remove_path("src/configfile-glue.c"), 498, dc->context_ndx, caches[dc->context_ndx].result == COND_RESULT_TRUE ? "true" : "false");
	    }
	} else {
	    if (con->conf.log_condition_cache_handling) {
	        log_trace("%s.%d: (trace) [%d] (cached) result: %s", remove_path("src/configfile-glue.c"), 504, dc->context_ndx, caches[dc->context_ndx].result == COND_RESULT_TRUE ? "true" : "false");
	    }
	}
