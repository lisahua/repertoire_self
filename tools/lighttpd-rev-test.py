...
if (len(argv) < 8):
    ...
    old_rev = argv[6];
    deps_dir = argv[7];
out_file = "lighttpd-rev-"+old_rev + "-" + new_rev + ".txt";
...
setup_svn_repo(repo1, new_rev);
setup_svn_repo(repo2, old_rev);

build_repo(repo1, build_cmd, deps_dir);
build_repo(repo2, build_cmd, deps_dir);
